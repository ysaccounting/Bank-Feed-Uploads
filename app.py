...
def log_conversion(broker, platform, start_date, end_date, row_count):
...
def process(df: pd.DataFrame, config: dict, start: date, end: date) -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        ...
        if config.get("wex_last4"):
            ...
            for std_col, alt_col in alt_col_map.items():
            ...
            if df is None:
                ...
                if result.empty:
                    ...
                    if account_name.strip():
                        platform_clean = platform.replace(" ", "_") + f"_({account_name.strip()})"
                    else:
                    ...
                    log_conversion(broker, platform, str(start_date), str(end_date), len(result))
