...
platform = st.selectbox("Platform", [""] + sorted(PLATFORM_CONFIG.keys()))
...
output_mode = st.radio("Output files", ["Single file", "Multiple files"], horizontal=True)
rows_per_file = None
if output_mode == "Multiple files":
    rows_per_file = st.radio("Rows per file", [50, 75, 100], horizontal=True)

st.divider()
uploaded_file = st.file_uploader("Source file", type=["csv", "xlsx"])

if st.button("Convert", type="primary", disabled=not all([broker, platform, start_date, end_date, uploaded_file])):
