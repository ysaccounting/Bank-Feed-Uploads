...
def process(df: pd.DataFrame, config: dict, start: date, end: date) -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        ...
        # Format date
        formatted_date = tx_date.strftime("%-m/%-d/%Y") if hasattr(tx_date, 'strftime') else str(tx_date)
...
def reset():
    ...
    for key in ["broker", "platform", "start_date", "end_date", "output_mode", "rows_per_file"]:
...
ready = all([broker, platform, start_date, end_date, uploaded_file])
convert_color = "#2e7d32" if ready else "#a5d6a7"
convert_cursor = "pointer" if ready else "not-allowed"
convert_disabled = "" if ready else "disabled"
convert_events = "" if ready else "pointer-events:none;"

btn_html = f"""
<style>
.bfu-btn-row {{ display: flex; gap: 12px; margin-top: 0.5rem; }}
.bfuconvert {{
    flex: 2; padding: 10px 0;
    background: {convert_color}; color: white;
    border: none; border-radius: 8px;
    font-size: 14px; font-weight: 500;
    cursor: {convert_cursor}; {convert_events}
    width: 100%;
}}
.bfuclear {{
...
#bfuconvert-hidden, #bfuclear-hidden {{ display: none; }}
</style>
<div class="bfu-btn-row">
  <button class="bfuconvert" {convert_disabled}
    onclick="window.parent.document.querySelectorAll('button[data-testid=stBaseButton-secondary]').forEach(function(b){{if(b.innerText.trim()=='__convert__')b.click();}})">
    Convert
  </button>
  <button class="bfuclear"
    onclick="window.parent.document.querySelectorAll('button[data-testid=stBaseButton-secondary]').forEach(function(b){{if(b.innerText.trim()=='__clear__')b.click();}})">
    Clear / Reset
  </button>
</div>
"""
...
convert_clicked = st.button("__convert__")
if st.button("__clear__"):
    reset()
    st.rerun()
