...
broker = st.text_input("Broker", key="broker")
platform = st.selectbox("Platform", [""] + sorted(PLATFORM_CONFIG.keys()), key="platform")
...
output_mode = st.radio("Output files", ["Single file", "Multiple files"], horizontal=True, key="output_mode")
rows_per_file = None
if output_mode == "Multiple files":
    rows_per_file = st.radio("Rows per file", [50, 75, 100], horizontal=True, key="rows_per_file")

st.divider()
uploaded_file = st.file_uploader("Source file", type=["csv", "xlsx"], key=f"uploader_{st.session_state['reset_counter']}")
